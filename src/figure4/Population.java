...
public class Population {
	...
	public String toString() {
		...
		for (Individual indiv : pop) {
			res += String.valueOf(cnt) + ". " + indiv.toString() + " (" + indiv.getTotalValue() + ")\n";
			cnt++;
		}
		return res;
	}
	...
	public double getAvrScore() {
		double total = 0;
		for (int i=0 ; i < populationSize ; i++){ total += (score[i]-1); }
		return total/populationSize;
	}
	
	public double getAvrSubjFitness() {
		return this.getAvrScore()/sampleSize;
	}
	
	public double getAvrObjFitness() {
		double total = 0;
		for (int i=0 ; i < populationSize ; i++){ total += pop.get(i).getTotalValue(); }
		return total/populationSize;
	}
}
